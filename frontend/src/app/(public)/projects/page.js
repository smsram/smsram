...
export default function PublicDirectoryPage() {
  ...
  const currentCategoryType = useMemo(() => {
    ...
    if (pathname.includes('/source-code')) return 'SOURCE';
  ...
  const directoryMeta = useMemo(() => {
    const metaDefinitions = {
      PROJECT: { title: "Production Ecosystem", sub: "Deployed products and live systems architecture directory", path: "projects" },
      VIDEO: { title: "Broadcast Systems", sub: "Video walk-through deep dives and systems engineering explanations", path: "videos" },
      SOURCE: { title: "Source Repositories", sub: "Modular isolated microservices and infrastructure codebases directory", path: "source-code" },
      BLOG: { title: "Core Transmissions", sub: "Architectural design entries and publication notes ledger", path: "blog" },
      SERVICE: { title: "Public Utilities", sub: "Operational live sandbox API endpoints and tool layers", path: "services" }
    };
    return metaDefinitions[currentCategoryType];
  }, [currentCategoryType]);
  ...
  const fetchInventoryBatch = useCallback(async (pageToLoad, isNewSearch = false) => {
    ...
    try {
      ...
      console.error("Infrastructure loading exception captured:", err);
    } finally {
      setIsLoading(false);
      setIsLoadingMore(false);
    }
  }, [currentCategoryType, searchQuery, selectedStack]);

  useEffect(() => {
    setCurrentPage(1);
    fetchInventoryBatch(1, true);
  }, [searchQuery, selectedStack, currentCategoryType, fetchInventoryBatch]);
  ...
  const processedCardsList = useMemo(() => {
    ...
    return rawAssets.map((asset, index) => {
      ...
      return {
        id: asset.id,
        title: asset.title,
        description: parsedMeta?.summary || 'No overview summary text declared for this component item.',
          ...
          <div style={{ display: 'flex' }} className="items-center gap-4 w-full flex-col sm:flex-row">
            ...
            <div style={{ display: 'block' }} className="w-full sm:flex-1">
              <Input 
                ...
                placeholder={`Search ${directoryMeta.path} logs...`}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full animate-in fade-in duration-300">
                {processedCardsList.map((project) => (
                  <DirectoryCard 
                    ...
                    href={`/${directoryMeta.path}/${project.slug}`}
                  />
                ))}
              </div>
